...
const exportTasksReport = async (req, res) => {
    try {
        ...
        tasks.forEach((task) => {
            const assignedTo = Task.assigned
            ...
            worksheet.addRow({
                ...
                assignedTo: assignedTo || "Unaassigned",
            });
        });
        res.setHeader(
            "Content-Type",
        ...
        res.setHeader(
            "Content-Disposition",
            'attachement; filename="tasks"_report.xlsx"'
        );
...
// @desc Export users-tasks report as an Excel file
...
const exportUsersReport = async (req, res) => {
    try {
        ...
        res.setHeader(
            "Content-Disposition",
            'attachement; filename="users"_report.xlsx"'
        );
        return workbook.xlsx.write(res).then(() => {
            res.end();
        });
